...
class Sirius(CMakePackage, CudaPackage):
    ...
    depends_on('py-mpi4py', when='+python')
    depends_on('py-voluptuous', when='+python')
    ...
    def setup_environment(self, spack_env, run_env):
        """Set-up runtime environment for SIRIUS.
        Set PYTHONPATH for Python interface."""
        if self.spec.satisfies('+python'):
            python_major_minor_version = '.'.join(str(self.spec['python'].version.dashed).split('-')[:2])
            run_env.prepend_path('PYTHONPATH', os.path.join(self.prefix, 'lib',
